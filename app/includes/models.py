...
def get_dict_to_sparse(d1, shape=None):
    a = []
    for k, v in d1.items():
    ...
    if shape is None:
        ...
        return sp.csr_matrix((np.ones(a.shape[0]), (a[:, 0], a[:, 1])), shape=shape)
